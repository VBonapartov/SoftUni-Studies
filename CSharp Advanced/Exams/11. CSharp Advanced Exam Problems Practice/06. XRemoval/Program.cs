    ...
    class Program
    {
        ...
        private static List<int[]> PlusRemove()
        {
            ...
            for (int row = 0; row < matrix.Length; row++)
            {
                ...
                {
                    if (IsXCenter(row, col))
                    {
                        removedCells.Add(new[] { row, col });
                        removedCells.Add(new[] { row - 1, col - 1 });
                        removedCells.Add(new[] { row - 1, col + 1 });
                        removedCells.Add(new[] { row + 1, col - 1 });
                        removedCells.Add(new[] { row + 1, col + 1 });
                    }
        ...
        private static bool IsXCenter(int row, int col)
        {
            char symbol = Char.ToLower(matrix[row][col]);

            if (row - 1 < 0 || col - 1 < 0 || col + 1 > matrix[row - 1].Length - 1 ||
                row + 1 > matrix.Length - 1 || col + 1 > matrix[row + 1].Length - 1)
            {
                return false;
            }

            return Char.ToLower(matrix[row - 1][col - 1]).Equals(symbol) &&
                   Char.ToLower(matrix[row - 1][col + 1]).Equals(symbol) &&
                   Char.ToLower(matrix[row + 1][col - 1]).Equals(symbol) &&
                   Char.ToLower(matrix[row + 1][col + 1]).Equals(symbol);
        }
