    ...
    public class GarbageProcessor : IGarbageProcessor
    {
        ...
        public IProcessingData ProcessWaste(IWaste garbage)
        {
            ...
            if (!this.StrategyHolder.GetDisposalStrategies.ContainsKey(typeOfAttribute))
            {
                ...
                IGarbageDisposalStrategy activateStrategy = (IGarbageDisposalStrategy)Activator.CreateInstance(typeOfCorrespondingStrategy);

                this.StrategyHolder.AddStrategy(typeOfAttribute, activateStrategy);
            }
